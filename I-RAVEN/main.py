...
def separate(args, all_configs):
    ...
    ood_attribute_idx = -1
    for i, is_iid_attribute in enumerate([args.position, args.type, args.size, args.color]):
    ...
    for configuration in all_configs.keys():
        ...
        for k in range(args.num_samples):
            ...
            while True:
                rule_groups = sample_rules(num_components, contains_mesh_component, configuration, ood_attribute_idx, set_name)
            ...
            imgs = [render_panel(row_1_1, should_render_random_mesh_component),
            ...
            if len(selected_attr) >= 3:
                ...
                for i in range(num_attributes_to_modify):
                    component_idx, attr_name, _, min_level, max_level, attr_uni = selected_attr[i]
                ...
                selected_attr[0][3], selected_attr[0][4], selected_attr[0][5]
                ...
                component_idx, attr_name, min_level, max_level, attr_uni = selected_attr[1][0], selected_attr[1][1], \
                    selected_attr[1][3], selected_attr[1][4], selected_attr[1][5]
                if mode == 'Position-Number':
                    ran, qu = 6, 1
                else:
                    ran, qu = 3, 2
            ...
            for candidate in candidates:
                answers.append(render_panel(candidate, should_render_random_mesh_component))
            ...
            np.savez("{}/{}/RAVEN_{}_{}.npz".format(args.save_dir, configuration, k, set_name), image=image,
                     target=target,
                     predict=target,
            ...
            with open("{}/{}/RAVEN_{}_{}.xml".format(args.save_dir, configuration, k, set_name), "wb") as f:
...
def select_modifiable_attributes(
    ...
    if num_attributes_to_modify < len(modifiable_attributes):
        if contains_mesh_component:
            ...
            num_selected_mesh_attributes = np.random.randint(1, len(modifiable_mesh_attributes) + 1)
            selected_mesh_attribute_indices = np.random.choice(
                len(modifiable_mesh_attributes),
                num_selected_mesh_attributes,
                replace=False)
            selected_attributes += [modifiable_mesh_attributes[i] for i in selected_mesh_attribute_indices]
            ...
            num_selected_non_mesh_attributes = num_attributes_to_modify - num_selected_mesh_attributes
            selected_non_mesh_attribute_indices = np.random.choice(
                len(modifiable_non_mesh_attributes),
                num_selected_non_mesh_attributes,
                replace=False)
            selected_attributes += [modifiable_non_mesh_attributes[i] for i in selected_non_mesh_attribute_indices]
            return selected_attributes
        else:
            idx = np.random.choice(len(modifiable_attributes), num_attributes_to_modify, replace=False)
...
def main():
    ...
    main_arg_parser.add_argument("--save-dir", type=str, default="~/datasets/I-RAVEN",
    ...
    main_arg_parser.add_argument("--configurations", type=str, default='center_single,distribute_four,distribute_nine,left_center_single_right_center_single,up_center_single_down_center_single,in_center_single_out_center_single,in_distribute_four_out_center_single')
    ...
    all_configs = {
        ...
        if k in args.configurations.split(',')
    }
